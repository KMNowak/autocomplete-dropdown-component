...
As a result cannot define explicitly when the component should be updated regarding `nextProps` and `nextState`, but the shallow comparison is performed.
...
reference to passed object did not change so it will memoize it's form and prevent rerender.
...
1. In perfect scenario firstly we should define snapshot tests for the component that is being rewritten. For all interactions that cause rerenders. Thanks to that we'll be 100% certain that new `React.FC` is 1:1 with the old one.
